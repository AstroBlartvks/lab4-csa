...
- **Литералы:**
    ...
    - строки (`." ..."`, `S" ..."`) размещаются в data-секции как `cstr` (null-terminated, **1 символ = 1 машинное слово** — требование варианта);
...
Фон Нейман: единое 32-битное адресное пространство, пословная адресация. Память — однопортовая. Доступ — через **кеш**, на промах +9 тактов.
...
- **Числовой литерал** → `LIT` + значение в следующем слове памяти инструкций. Адресация к data-секции не нужна.
- **Строковый литерал** → размещается в data-секции один раз, в позиции появления. cstr: каждый символ — одно слово, терминатор — нулевое слово.
- **Переменная (`variable name`)** → выделяется одно слово в data-секции; имя в словаре превращается в push адреса этой ячейки. Чтение — `x @`, запись — `value x !`.
- **Константа (`constant name`)** → фиксируется на этапе трансляции; в памяти не хранится. Использование компилируется как `LIT <value>`.
- **`allot N`** → выделяет дополнительные N слов после предыдущей `variable`, превращая её в массив.
...
| **Стек данных** ||||
| `LIT` | 0x10 | → n | push константы (значение в след. слове) | 2 |
| `DUP` | 0x11 | a → a a | дубль вершины | 1 |
| `DROP` | 0x12 | a → | снять | 1 |
| `SWAP` | 0x13 | a b → b a | обмен двух вершин | 2 |
| `OVER` | 0x14 | a b → a b a | копия второй сверху | 2 |
| **АЛУ** ||||
| `ADD` `SUB` `MUL` `DIV` `MOD` | 0x20–0x24 | a b → r | бинарные | 1 |
| `AND` `OR` `INVERT` | 0x27, 0x28, 0x29 | — | побитовые | 1 |
| `EQ` `LT` `GT` | 0x25, 0x26, 0x2A | a b → flag | сравнения (Forth: -1/0) | 1 |
| **Память (через кеш)** ||||
| `LOAD` (`@`) | 0x30 | addr → val | чтение из памяти | 2+M |
| `STORE` (`!`) | 0x31 | val addr → | запись в память | 2+M |
| **Поток управления** ||||
| `JMP a` | 0x40 | — | безусловный переход на адрес `a` | 1 |
| `JZ a` | 0x41 | flag → | переход если pop == 0 | 1 |
| `CALL a` | 0x42 | — | push PC+1 на RS; PC ← a | 1 |
| `RET` | 0x43 | — | PC ← pop с RS | 1 |
| `EXECUTE` | 0x44 | xt → | call по адресу с вершины DS | 1 |
| **Стек возвратов** ||||
| `>R` | 0x50 | a → ⟨RS: a⟩ | переслать с DS на RS | 1 |
| `R>` | 0x51 | ⟨RS: a⟩ → a | переслать с RS на DS | 1 |
| `R@` | 0x52 | → a | копия вершины RS на DS | 1 |

\* Указаны такты только тела микропрограммы (без FETCH). FETCH занимает ещё 2 такта без учёта cache stall'ов. M = +9 тактов на miss, +9 ещё на write-back грязной строки при вытеснении.
...
- `operand` (24 бита) → адрес перехода для `JMP`/`JZ`/`CALL`.
- Для инструкций без операнда поле заполняется нулями.
- **Спецслучай `LIT`:** опкод-слово + следующее слово, содержащее 32-битное значение. При FETCH процессор увидит `LIT` и в микропрограмме `LIT` сам прочитает ещё одно слово и инкрементирует `PC` дважды.
...
Пример дампа (формат `<address> - <HEXCODE> - <mnemonic>`):
...
Архитектурно состоит из четырёх частей:
...
| Алгоритм | LoC | Размер кода (слов) | Тактов | Cache: hits / misses / writebacks |
|---|---|---|---|---|
| hello | 1 | 103 | 754 | 145 / 29 / 0 |
| cat | 1 | 88 | 145 | 36 / 3 / 0 |
| hello_user_name | 22 | 211 | 3684 | 509 / 180 / 10 |
| sort | 66 | 271 | 1228 | 184 / 56 / 0 |
| double_precision | 111 | 846 | 94131 | 6831 / 5916 / 386 |
| prob1 | 30 | 224 | 6172259 | 555357 / 376512 / 14945 |
| execute_demo | 8 | 132 | 1493 | 236 / 67 / 1 |
| cache_demo | 25 | 216 | 20885 | 1410 / 1336 / 101 |

LoC — непустые строки исходника без строк-комментариев (`\`). Размер кода
включает рантайм-функции (print_cstr, print_num) — поэтому даже минимальные
программы занимают ~90–150 слов.
...
DEBUG   machine:simulation    TICK:   1 PC:   1 mPC: 1 TOS:         0 NOS:         0
DEBUG   machine:simulation    TICK:   2 PC:   1 mPC: 1 TOS:         0 NOS:         0
...
DEBUG   machine:simulation    TICK:  12 PC:   1 mPC:27 TOS:         0 NOS:         0
DEBUG   machine:simulation    TICK:  13 PC:  81 mPC: 0 TOS:         0 NOS:         0  JMP
...
Hello, World!
ticks: 836
```
